...
# Postgres: JSONB é melhor, mas mantém fallback.
try:
...
class EntityType(str, enum.Enum):
    # NOTE: the existing Postgres enum in production uses "INSURANCE".
    # We keep INSURER as an alias for backward compatibility with older code/clients.
    INSURANCE = "INSURANCE"
    INSURER = "INSURANCE"
    BANK = "BANK"
    OTHER = "OTHER"
...
class Source(Base):
    ...
    name = Column(String, nullable=False)
    category = Column(String, nullable=False)  # e.g. PEP / SANCTIONS / WATCHLIST / UNDERWRITING
    collected_from = Column(String, nullable=False)  # origem / provider
...
class Risk(Base):
    ...
    # Resultado (mock por agora / pronto para motor real)
    score = Column(String, nullable=True)
    summary = Column(Text, nullable=True)
    matches = Column(JSON, nullable=False, default=list)  # lista de matches normalizáveis

    status = Column(Enum(RiskStatus), nullable=False, default=RiskStatus.DRAFT)
    created_by = Column(String, ForeignKey("users.id"), nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow)

    # Underwriting (agregado/derivado, opcional)
...
# =====================================================================
# Underwriting / Insurance tables (multi-fonte, por product_type)
# =====================================================================
...
class Payment(Base):
    ...
    status = Column(String, nullable=True)  # PAID / LATE / FAILED
...
class Claim(Base):
    ...
    status = Column(String, nullable=True)  # OPEN / CLOSED / REJECTED
