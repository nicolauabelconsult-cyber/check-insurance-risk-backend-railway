...
def _resolve_entity_id(u: User, requested: str | None) -> str:
    # Produção V1:
    # - SUPER_ADMIN pode operar por entity_id explícito
    # - ADMIN é scoped ao seu tenant (como cliente) e ignora entity_id externo
...
def confirm_risk(body: RiskConfirmIn, db: Session = Depends(get_db), u: User = Depends(require_perm("risk:confirm"))):
    ...
    # NOTE: search_id sequencial pode ser adicionado depois.
    r = Risk(
        id=str(uuid.uuid4()),
        entity_id=entity_id,
...
def risk_pdf(risk_id: str, db: Session = Depends(get_db), u: User = Depends(require_perm("risk:pdf:download"))):
    ...
    # 🔥 Aqui é onde os "dados de seguros" entram no PDF.
    # Se não houver dados carregados, isto devolve {} e o PDF vai escrever "Sem informações…".
    underwriting_by_product = load_underwriting_by_product(
        db=db,
        entity_id=r.entity_id,
        full_name=getattr(r, "query_name", None),
        bi=getattr(r, "query_bi", None),
        passport=getattr(r, "query_passport", None),
    )
    ...
    try:
        pdf_bytes = build_risk_pdf_institutional(
            ...
            compliance_by_category=None,  # usa matches do risk (normaliza)
        )
    ...
    return StreamingResponse(BytesIO(pdf_bytes), media_type="application/pdf", headers={"Content-Disposition": f"attachment; filename=risk_{r.id}.pdf"})
