...
def search_risk(
    ...
    """Cria um Risk (DRAFT) e devolve candidatos (vazio por agora).

    IMPORTANTE: RiskSearchOut no teu schemas.py tem APENAS:
      - disambiguation_required
      - candidates
    Se devolvermos qualquer outro campo (ex: risk=...), o FastAPI levanta ValidationError -> 500.
    """
    ...
    # ✅ resposta 100% compatível com RiskSearchOut (mata o 500)
...
def confirm_no_match(
    ...
    Suporta candidate_id == 'NO_MATCH' e gera um Risk DONE, pronto para PDF.
    """
    entity_id = payload.entity_id or getattr(user, "entity_id", None)
    if not entity_id:
        raise HTTPException(status_code=400, detail="entity_id required")
...
def confirm_risk_legacy(
    ...
    """Mantido para compatibilidade (fluxos antigos)."""
