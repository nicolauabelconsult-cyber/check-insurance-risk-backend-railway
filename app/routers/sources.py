...
def list_sources(
    ...
    u=Depends(require_perm("sources:read")),
):
    """Lista fontes.

    - CLIENT/ADMIN (com entity_id fixo): sempre limitado à sua entidade.
    - SUPER_ADMIN (sem entity_id fixo): pode passar ?entity_id=... para alternar.
    """

    resolved = resolve_entity_id(u, entity_id, require=False)
    q = db.query(Source)

    if resolved:
        q = q.filter(Source.entity_id == resolved)
