...
def create_user(body: UserCreate, db: Session = Depends(get_db), u=Depends(require_perm("users:create"))):
    # entity_id obrigatório na criação (se quiseres permitir omitir para CLIENT_ADMIN, fazemos depois)
...
def update_user(user_id: str, body: UserUpdate, db: Session = Depends(get_db), u=Depends(require_perm("users:update"))):
    ...
    if body.role is not None:
        ...
        if (
            u.role not in {UserRole.SUPER_ADMIN, UserRole.ADMIN}
            and body.role in {UserRole.SUPER_ADMIN.value, UserRole.ADMIN.value}
        ):
