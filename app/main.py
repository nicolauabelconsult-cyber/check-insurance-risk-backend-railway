...
def ensure_sector_entities():
    """Ensure the 5 sector-tenants exist as Entities.

    We keep the existing Entity table to avoid schema churn in PROD V1.
    Entities are treated as 'sectors' for now (name in ALLOWED_SECTORS).
    """
    db = SessionLocal()
    try:
        existing = {e.name for e in db.query(Entity).all()}
...
def create_app() -> FastAPI:
    ...
    def _startup():
        # Produção V1: usamos Entities como "Sectors" (tenants) por enquanto.
        ensure_sector_entities()
