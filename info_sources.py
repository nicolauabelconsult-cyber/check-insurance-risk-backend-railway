# info_sources.py – versão sem pandas, compatível com frontend
...
async def upload_info_source_excel(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Erro ao ler Excel: {exc}",
        )
    ...
    if idx_name is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="A coluna 'name' é obrigatória.",
        )
    ...
    # 4) Criar entidades normalizadas
    created = 0
    for row in rows[1:]:
        if not row:
            continue
        ...
        entity = NormalizedEntity(
            ...
            country=(
                _str_or_none(row[idx_country]).upper()
